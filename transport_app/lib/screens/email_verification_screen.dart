...
class _EmailVerificationScreenState extends State<EmailVerificationScreen> {
  ...
  void _startPolling() {
    _pollTimer = Timer.periodic(const Duration(seconds: 5), (_) async {
      ...
      if (FirebaseAuth.instance.currentUser?.emailVerified == true) {
        _pollTimer?.cancel();
        if (mounted) {
          // Go back to root — AuthWrapper routes user to the app
  ...
  Future<void> _checkNow() async {
    ...
    try {
      ...
      if (FirebaseAuth.instance.currentUser?.emailVerified == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(AppLocalizations.of(context).emailNotYetVerified),
          backgroundColor: Theme.of(context).colorScheme.error,
          behavior: SnackBarBehavior.floating,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
          margin: const EdgeInsets.all(16),
  ...
  Future<void> _backToSignIn() async {
    ...
    // Pop back to root — AuthWrapper now sees no user → shows LoginScreen
    if (mounted) Navigator.of(context).popUntil((route) => route.isFirst);
  }
