...
class _MapScreenState extends State<MapScreen> with TickerProviderStateMixin {
  ...
  static const _gYellow = Color(0xFFFBBC05);
  static const _gDark = Color(0xFF202124);
  static const _gText = Color(0xFF3C4043);
  static const _gSub = Color(0xFF5F6368);
  static const _gLight = Color(0xFFF1F3F4);
  static const _gBorder = Color(0xFFDADCE0);
  ...
  @override
  void dispose() {
    _busLocationSub?.cancel(); _myPositionSub?.cancel(); _etaRefreshTimer?.cancel(); _panelAnim.dispose();
    super.dispose();
  }
  ...
  Future<void> _initLocation() async {
    try {
      ...
      if (mounted) { setState(() => _myPosition = LatLng(pos.latitude, pos.longitude)); _calcMeToBus(); }
      _myPositionSub = Geolocator.getPositionStream(locationSettings: const LocationSettings(accuracy: LocationAccuracy.high, distanceFilter: 10))
          .listen((p) { if (mounted) { setState(() => _myPosition = LatLng(p.latitude, p.longitude)); _calcMeToBus(); } });
    } catch (_) {}
  }
        ...
        body: _isLoading
          ...
          FlutterMap(
            ...
            children: [
              ...
              // Done route (grey like Google)
              if (_doneRoute.length >= 2)
                PolylineLayer(polylines: [
                  Polyline(points: _doneRoute, color: const Color(0xFFBDC1C6), strokeWidth: 8),
                ]),

              // Remaining route (Google blue)
              if (_leftRoute.length >= 2)
                PolylineLayer(polylines: [
                  Polyline(points: _leftRoute, color: _gBlue, strokeWidth: 8),
                ]),

              // Fallback
              if (!_routeOk && live && bus.hasArrival)
                PolylineLayer(polylines: [
                  Polyline(points: [_busLocation!.latLng, LatLng(bus.arrivalLat!, bus.arrivalLng!)], color: _gBlue.withValues(alpha: 0.5), strokeWidth: 4),
                ]),

              // Markers
              MarkerLayer(markers: [
                // Departure — Google green circle
                if (bus.hasDeparture)
                  Marker(point: LatLng(bus.departureLat!, bus.departureLng!), width: 18, height: 18,
                      child: Container(decoration: BoxDecoration(color: _gGreen, shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 2)))),

                // Arrival — Google red pin
                if (bus.hasArrival)
                  Marker(point: LatLng(bus.arrivalLat!, bus.arrivalLng!), width: 36, height: 42,
                      child: Icon(Icons.location_on, color: _gRed, size: 42)),
                ...
                // Bus — orange circle with bus icon
                if (live)
                  Marker(point: _busLocation!.latLng, width: 40, height: 40,
                      child: Container(
                          decoration: BoxDecoration(
                            color: Colors.white,
                            shape: BoxShape.circle,
                  ...
                  Marker(point: _tapPt!, width: 130, height: 56, child: Column(mainAxisSize: MainAxisSize.min, children: [
                    Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
                        ...
                        child: _tapLoading
                          ...
                          if (_tapDist != null) ...[
                            Text('  ·  ', style: TextStyle(color: _gSub, fontSize: 10)),
                            Text(_tapDist!, style: TextStyle(fontSize: 10, color: _gSub)),
                          ],
                        ])),
                    Container(width: 8, height: 8, margin: const EdgeInsets.only(top: 2),
                        decoration: BoxDecoration(color: _gBlue, shape: BoxShape.circle)),
                  ])),
              ]),
            ],
          ),
          ...
          // ═══════════════════════════════════════
          // TOP: Back + Search-like bar
          // ═══════════════════════════════════════
          Positioned(top: 0, left: 0, right: 0,
              child: Container(
                padding: EdgeInsets.fromLTRB(8, top + 8, 16, 8),
                child: Row(children: [
                  // Back button (Google style circle)
                  Material(
                    color: Colors.white,
            ...
            Positioned(top: top + 72, left: 16, right: 16,
                child: Material(
                  ...
                  child: Padding(
                    ...
                    child: Row(children: [
                      const Icon(Icons.info_outline, color: _gYellow, size: 18),
                      const SizedBox(width: 10),
                      const Text('Bus pas en trajet actuellement', style: TextStyle(fontSize: 13, color: _gText)),
                    ]),
                  ),
                )),

          // ═══════════════════════════════════════
          // ETA HEADER (Google Maps style green/red bar)
          // ═══════════════════════════════════════
                  ...
                  child: Padding(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Text('$_distText  ·  Arrivée $_arrText', style: TextStyle(fontSize: 12, color: Colors.white.withValues(alpha: 0.9))),
                      ])),
                      // Me to bus
                      if (_myPosition != null)
                        Container(
          ...
          // ═══════════════════════════════════════
          // BOTTOM PANEL (Google style white card)
          // ═══════════════════════════════════════
          Positioned(bottom: 0, left: 0, right: 0,
              child: SlideTransition(position: _panelSlide,
                  child: Container(
                    decoration: BoxDecoration(
                      color: Colors.white,
                    ...
                    child: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      Padding(padding: const EdgeInsets.all(16), child: Row(children: [
                        ...
                        Container(width: 44, height: 44,
                            decoration: BoxDecoration(color: _gBlue.withValues(alpha: 0.1), shape: BoxShape.circle),
          ...
          // ═══════════════════════════════════════
          // RIGHT CONTROLS (Google style)
          // ═══════════════════════════════════════
          Positioned(bottom: live ? 160 : 80, right: 16,
              child: Column(children: [
                // Fit all
                ...
                // Center on bus
                if (live) ...[
                  _GButton(Icons.directions_bus, () { _followBus = true; _mapController.move(_busLocation!.latLng, 16); }),
                  const SizedBox(height: 10),
                ],
                // My location (Google blue tint)
                if (_myPosition != null)
                  _GButton(Icons.my_location, () { _followBus = false; _mapController.move(_myPosition!, 16); }, tint: _gBlue),
              ])),
        ]),
      ),
    );
